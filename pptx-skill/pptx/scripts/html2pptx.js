  ...
  if (slideData.background.type === 'image' && slideData.background.path) {
    let imagePath = slideData.background.path;
      if (imagePath.startsWith('file://')) {
        imagePath = decodeURIComponent(imagePath.slice(8));
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      if (el.animate) imgOptions.animate = el.animate;

      if (el.objectFit === 'contain' || el.objectFit === 'cover') {
        imgOptions.sizing = { type: el.objectFit, w: el.position.w, h: el.position.h };
      } else {
        imgOptions.w = el.position.w;
        imgOptions.h = el.position.h;
      }

      targetSlide.addImage(imgOptions);
      ...
      if (el.animate) lineOptions.animate = el.animate;
      ...
      if (el.animate) shapeOptions.animate = el.animate;
      ...
      if (el.animate) listOptions.animate = el.animate;
      ...
      // Make single-line text 5% wider to account for underestimate
      if (isSingleLine) {
      ...
      if (el.animate) textOptions.animate = el.animate;
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const parseInlineFormatting = (element, baseOptions = {}, runs = [], baseTextTransform = (x) => x, processedSet = null) => {
      ...
      element.childNodes.forEach((node) => {
        ...
        if (isText) {
          ...
          if (node.tagName === 'SPAN' || node.tagName === 'B' || node.tagName === 'STRONG' || node.tagName === 'I' || node.tagName === 'EM' || node.tagName === 'U') {
            if (processedSet) processedSet.add(node);
            ...
            parseInlineFormatting(node, options, runs, textTransform, processedSet);
          }
        }
    ...
    // Helper to extract animation data
    const getAnimation = (el) => {
      const anim = el.getAttribute('data-anim');
      if (!anim) return undefined;
      const order = el.getAttribute('data-anim-order');
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      // (Excluding SPAN from this check as it's often used for chips/inline labels)
      ...
      if (!textTags.includes(el.tagName)) {
        if (el.tagName !== 'DIV' && el.tagName !== 'IMG' && el.tagName !== 'HTML' && el.tagName !== 'BODY' && el.tagName !== 'HEAD' && el.tagName !== 'STYLE' && el.tagName !== 'SCRIPT' && el.tagName !== 'META' && el.tagName !== 'LINK' && el.tagName !== 'BR' && el.tagName !== 'B' && el.tagName !== 'STRONG' && el.tagName !== 'I' && el.tagName !== 'EM' && el.tagName !== 'U' && el.tagName !== 'SPAN') {
           // Skip logging for common structural and inline tags
        }
        return;
      }
      ...
      if (hasFormatting) {
        ...
        const transformStr = computed.textTransform;
        const runs = parseInlineFormatting(el, {}, [], (str) => applyTextTransform(str, transformStr), processed);
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  // Track whether we own the browser (launched it ourselves) so we can close it
  ...
  // Resolve to a browser instance (shared or newly launched)
  let browserInstance;
  if (sharedBrowser) {
    browserInstance = sharedBrowser;
  } else {
    const launchOptions = { env: { TMPDIR: tmpDir } };
    if (process.platform === 'darwin') launchOptions.channel = 'chrome';
    ownedBrowser = await chromium.launch(launchOptions);
    browserInstance = ownedBrowser;
  }
  ...
  try {
    ...
    // Build a proper file:// URL with forward slashes (critical on Windows)
    ...
    // If page operations failed, propagate immediately
    ...
    // Determine if we need progressive disclosure slides
    const animOrders = slideData.elements
      .map(el => (el.animate && el.animate.order !== undefined) ? el.animate.order : null)
      .filter(order => order !== null);
    
    const maxOrder = animOrders.length > 0 ? Math.max(...animOrders) : 0;
    
    // If no animations, just render 1 slide. If maxOrder = 3, we might need up to 4 slides (0, 1, 2, 3) 
    // or just render a sequence where each step adds the next order.
    // Let's assume order=1 is the first animation, so we need a base slide (no anims), then order 1, etc.
    // But if elements with animations are essential, maybe the user wants them invisible on step 0.
    
    const uniqueOrders = Array.from(new Set(animOrders)).sort((a, b) => a - b);
    const steps = uniqueOrders.length > 0 ? [0, ...uniqueOrders] : [0];

    let lastTargetSlide = null;

    for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
      const currentStepOrder = steps[stepIndex];
      
      // We only use the passed-in 'slide' for the FIRST step. Subsequent steps MUST create new slides.
      const targetSlide = (stepIndex === 0 && slide) ? slide : pres.addSlide();
      lastTargetSlide = targetSlide;

      await addBackground(slideData, targetSlide, tmpDir);

      // Filter elements: 
      // Include element IF it has NO animation (always visible)
      // OR IF it has animation and its order <= currentStepOrder
      const stepSlideData = {
        ...slideData,
        elements: slideData.elements.filter(el => {
          if (!el.animate || el.animate.order === undefined) return true;
          return el.animate.order <= currentStepOrder;
        })
      };

      addElements(stepSlideData, targetSlide, pres);
    }

    return { slide: lastTargetSlide, placeholders: slideData.placeholders };
  } catch (error) {
