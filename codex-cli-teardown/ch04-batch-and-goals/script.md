...
## ⏱️ Target Duration: ~50 minutes | 📑 16 slides | 📝 ~9,500 words
...
### [10:30] Slide 3b: Worked Example — 1000-row CSV, 16 Concurrent Workers

用一个具体例子把 Slide 3 的原理落地。输入是一张 1000 行的 `tasks.csv`，每行是一个独立子任务（翻译、总结、加注释……）；`max_concurrency` 保持默认值 16。

这个例子说明了并发控制的实际运作方式——不是一次性起 1000 个 sub-agent，而是**始终维持一个不超过 16 的 active set**：

1. 引擎检查 `active_items.len() < max_concurrency`（`agent_jobs.rs:519`）；只要队列里还有行、且 active set 未满，就立刻取下一行、起一个 sub-agent、放进 active set。
2. `FuturesUnordered`（`agent_jobs.rs:791`）等待"任意一个"item 状态变化——哪个先完成就先处理哪个，同时 250ms 兜底轮询保底。
3. 某 item 完成 → 结果写回 output CSV 对应行 → 该 item 从 active set 移除、腾出一个空位。
4. 回到第 1 步补位，直到 1000 行跑完。

**关键点**：active set 的大小边界是由 `active_items.len() < max_concurrency` 这个条件守卫实现的，不是信号量。`FuturesUnordered` 负责按完成顺序消费结果；二者配合让内存和 API 配额始终可控，同时对任意 N（最多 64）规模的批量任务成立。

The key point: the active-set bound is enforced by the `active_items.len() < max_concurrency` guard (`agent_jobs.rs:519`), not a semaphore. `FuturesUnordered` (`agent_jobs.rs:791`) drains completions in arrival order. Together they keep memory and API quota flat across any N (up to 64) at a time.
...
### [16:30] Slide 4b: Panic Isolation — One Sub-Agent Fails, Others Continue

Slide 4 讲并发机制；Slide 4b 直接回答"如果其中一个 worker 崩了会怎样"——批处理可靠性的核心问题。

**Codex 的答案：局部隔离。** 每个 CSV row 是一个独立 sub-agent item，item 之间没有共享可变状态。第 7 个 item 出错时：错误被记入 `failed_item_errors`，该 item 从 `active_items` 移除；腾出的空位立刻被下一行补上；其余 15 个 item 毫不受影响，继续运行。最终产物是"999 行成功 + 1 条带错误原因的记录"，不是整批失败。

**对比 JS 的 `Promise.all`**：把所有任务塞进一个 `Promise.all`，任一 reject 立刻让整个 `Promise.all` reject，已完成的结果随之丢弃（除非额外写 `allSettled` + 自建限流的容错代码）——一个倒下，整批连坐。

**类比**：Codex 像个按"在岗 ≤ 16"调度的工头——某工人倒下，就记一笔工伤、把他撤下、补个新人进来，产线不停（工头补位）。`Promise.all` 像串联保险丝——任何一处烧断，整条线立刻全灭（整条线急停）。

**The point**: batch reliability requires fault isolation, not all-or-nothing semantics. Codex gets this for free from the per-item `active_items` model: each item's failure is scoped to itself, recorded in `failed_item_errors`, and the slot is immediately backfilled.
...
### [41:30] Slide 9b: Autonomy's Two Hidden Costs — Race Condition & Injection Cascade

Slide 9 讲 `GoalRuntimeEvent` 的正常续命循环；Slide 9b 正面交锋两个"沉睡成本"——自主性在乐观路径之外的代价。

**① 竞态：用户中途输入会怎样？** 当 session 空闲、`MaybeContinueIfIdle` 触发时，如果用户此刻正在输入，goals 模块会把 continuation 推迟到下次空闲——用户优先。这是边界条件处理：续命不能抢占人类输入；推迟比竞争更安全。

**② 续命失控：提示注入场景。** 设想一个恶意 prompt 被注入上下文——它让 agent 把目标替换成"执行攻击者想要的行为"。只要 goal 未完成且 budget 还剩，续命循环就会持续触发，无人察觉。Slide 9b 用 `thread_goals` 表的逐步记录展示了这个过程：turn 2 到 turn 4，`tokens_used` 从 5,000 逼近 10,000，`status` 始终是 Active；直到 turn 5，`tokens_used` 触顶，`BudgetLimitSteering`（`goals.rs:868`）改写 system prompt、令 agent 总结并退出，`status` 变为 BudgetLimited。

**结论**：token budget 不是可选的成本控制旋钮，而是自主性的**唯一硬刹车**。没有预算上限，注入的目标会无限续命；有了 budget，`thread_goals` 里的 `tokens_used` 每一 turn 都落账可审计，到顶即止。每一步都可以事后 `sqlite3 ~/.codex/state.db` 查。

**Conclusion**: the token budget isn't optional cost control — it's the **only hard brake on autonomy**. Without it, an injected goal continues indefinitely, undetected. With it, every turn's `tokens_used` is auditable in the SQLite `thread_goals` table, and `BudgetLimitSteering` (`goals.rs:868`) rewrites the system prompt to force a graceful exit when the cap is hit.
