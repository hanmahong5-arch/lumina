...
**CRITICAL — Aspect Ratio Constraint**: All diagrams are destined for 16:9 PowerPoint slides (720pt x 405pt). Plan layout to fit this ratio:
- Target aspect ratio (width/height): **1.4 to 2.0** (ideal: 1.78)
- Canvas bounds (max_x - min_x, max_y - min_y) must stay within this range
- **For pipeline/timeline diagrams**: use 2-row layouts, not a single long horizontal strip
- **For vertical flow charts**: widen the layout with side-by-side detail panels, or split into Part 1/Part 2
